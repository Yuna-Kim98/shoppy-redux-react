...
import { setCartCount, clearCartCount, setCartList, cartListReset, setTotalPrice, setIsAdded, isAddedReset } from "../features/cart/cartSlice.js";
/**
 * 장바구니 전체 카운트 조회
 */
...
export const updateCartList = (cid, type) => async(dispatch) => {
    ...
    // 수량 업데이트 성공 -> cartList 재호출
    // getCartList에 dispatch 객체가 넘어가고 있기 때문에 호출해서 사용하기 위해선 dispatch로 넘겨주어야 함
...
/**
 * 장바구니 새로운 아이템 저장 체크 초기화
 */
export const clearIsAdded = () => (dispatch) => {
    dispatch(isAddedReset());
}
